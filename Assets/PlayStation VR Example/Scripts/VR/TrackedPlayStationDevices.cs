...
namespace PlayStationVRExample
{
    public class TrackedPlayStationDevices : MonoBehaviour
    {
        ...
        IEnumerator RegisterDualShock4Controller()
        {
            ...
            {
                var deviceType = (PS4Input.DeviceType)PS4Input.GetDeviceTypeForHandle(handle);
                ...
                if (deviceType == PS4Input.DeviceType.DeviceUnknown)
                {
            ...
            var result = Tracker.RegisterTrackedDevice(PlayStationVRTrackedDevice.DeviceDualShock4, deviceDualShock4.handle, trackingType, trackerUsageType);
        ...
        IEnumerator RegisterAimController()
        {
            ...
            PS4Input.AimGetUsersHandles(1, handles);
            ...
            var result = Tracker.RegisterTrackedDevice(PlayStationVRTrackedDevice.DeviceAim, deviceAim.handle, trackingType, trackerUsageType);
        ...
        IEnumerator RegisterMoveControllers()
        {
            ...
            var result = Tracker.RegisterTrackedDevice(PlayStationVRTrackedDevice.DeviceMove, deviceMovePrimary.handle, trackingType, trackerUsageType);

            if (result == PlayStationVRResult.Ok)
            {
                var trackingStatusPrimary = new PlayStationVRTrackingStatus();
            ...
            if (deviceMoveSecondary.transform)
            {
                ...
                if (result == PlayStationVRResult.Ok)
                {
                    var trackingStatusSecondary = new PlayStationVRTrackingStatus();
