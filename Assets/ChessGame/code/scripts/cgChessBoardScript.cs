...
public class cgChessBoardScript : MonoBehaviour {
    ...
    /// </summary>
    public enum BoardMode { Undefined, PlayerVsPlayer, PlayerVsEngine, EngineVsPlayer, EngineVsEngine, PlayerAlone}
        ...
        if (highlightLegalMoves && playerCanMove) {
            ...
            foreach (cgSimpleMove move in moves) {
                if (_abstractBoard.SquareNames[move.from] == piece.square.uniqueName) {
                    if (move is cgCastlingMove) {//Highlighting rook instead of king destinatif when castling.
    ...
    private void _pieceUp(cgChessPieceScript piece) {
        ...
        if (highlightLastMove) {//revert colors if highlighting is active
